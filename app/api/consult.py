...
class DailyFortuneRequest(BaseModel):
    google_id: str
    iljin: dict  # 오늘의 일진 데이터 (hdganjee, kdganjee, day_ddi 등)
...
async def _stream_daily_groq(saju: dict, iljin: dict) -> AsyncGenerator[str, None]:
    """오늘의 운세 전용 Groq 스트리밍 (일진 기준)"""
    user_prompt = f"""사용자 사주: {json.dumps(saju, ensure_ascii=False, indent=2)}

오늘의 일진: {json.dumps(iljin, ensure_ascii=False, indent=2)}

오늘의 일진과 사용자 사주의 관계를 분석하여 오늘 하루의 운세를 풀이해주세요. 일진(오늘의 천간지지)이 사용자의 일간, 오행, 십신과 어떤 관계인지 살펴보고 조언해주세요."""
...
async def consult_daily(req: DailyFortuneRequest):
    ...
    return StreamingResponse(
        _stream_daily_groq(saju, req.iljin),
